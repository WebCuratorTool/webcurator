...
public class BrowseHelper {
     ...
     */
    public void fix(StringBuilder content, String contentType, long jobId, int harvestResultNumber, String currentResource) {
        String resourcePrefix = getResourcePrefix(jobId, harvestResultNumber);
            ...
            for (Pattern pattern : contentTypePatterns.get(contentType)) {
                ...
                while (matcher.find(idx)) {
                    ...
                    if (!"".equals(currentVal.trim())) {
                        String newVal = convertUrl(jobId, harvestResultNumber, currentResource, currentVal);
        ...
        if ("text/html".equals(contentType) || "application/javascript".equals(contentType)) {
            Map<String, String> tokens = fixTokens;
    ...
    public String convertUrl(long jobId, int harvestResultNumber, String currentResource, String urlToConvert) {
        //For embeded image
        if (!Utils.isEmpty(urlToConvert) && urlToConvert.startsWith("data:image")) {
            return urlToConvert;
        }

        String resourcePrefix = prefix + "/" + jobId + "/" + harvestResultNumber + "/?url=";
...
//        return s;
    ...
    public String getResourcePrefix(long jobId, int harvestResultNumber) {
        return prefix + "/" + jobId + "/" + harvestResultNumber + "/?url=";
    }


    public static BrowseHelper browseHelper(String browseHelperPrefix, boolean browseDoubleEscape) {
    ...
    public static RegexReplacer urlConversionReplacementsRegexReplacer() {
