...
<table width="100%" cellpadding="3" cellspacing="0" border="0">
		...
		<c:otherwise>
			...
			<tr>
				...
				<c:when test="${agent.value.memoryWarning == false && agent.value.acceptTasks}">
					<td class="annotationsLiteRow"><input type="image" src="images/targets-btn-allocate.gif" onclick="document.getElementById('doneButton').style.display='none';document.getElementById('allocAlert').innerHTML='Allocating to the harvest agent.  You can continue browsing.';javascript:document.harvestNow.<%=TargetInstanceCommand.PARAM_AGENT%>.value='<c:out value="${agent.value.name}"/>';"/></td>
  ...
  <tr>
	...
	<input id="doneButton" type="image" src="images/generic-btn-done.gif" onclick="javascript:document.harvestNow.<%=TargetInstanceCommand.PARAM_CMD%>.value='<%=TargetInstanceCommand.ACTION_ABORT%>';" />
	</td>
  </tr>
</table>
</form>
