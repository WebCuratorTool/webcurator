 ...
 * 1668 $
 */
public class CustomizedTagMagix {
    ...
    private static final int MIN_ATTR_LENGTH = 3;

    private static final HashMap<String, Pattern> pcPatterns = new HashMap<>();

    private static final HashMap<String, Pattern> wholeTagPatterns = new HashMap<>();

    private static final HashMap<String, Pattern> attrPatterns = new HashMap<>();
    ...
    public static final String ANY_TAGNAME = "[a-z]+";

    private static final String STYLE_ATTR_NAME = "style";

    private static final String ANY_ATTR_VALUE = QUOTED_ATTR_VALUE + "|"
            + APOSED_ATTR_VALUE + "|" + ESC_QUOTED_ATTR_VALUE + "|"
            + RAW_ATTR_VALUE;

    private static final String cssUrlPatString = "(?<!@namespace [\\w\\s]{0,16})url\\s*\\(\\s*([\\\\\"']*.*?[\\\\\"']*)\\s*\\)";

    private static final String cssImportNoUrlPatString = "@import\\s+([\"'].+?[\"'])";

    private static final Pattern cssImportNoUrlPattern = Pattern.compile(cssImportNoUrlPatString);

    private static final Pattern cssUrlPattern = Pattern.compile(cssUrlPatString, Pattern.CASE_INSENSITIVE);
...
//		markupTagREURIC(page,uriConverter,captureDate,baseUrl,cssImportPattern);
    ...
    public static void markupTagREURIC(StringBuilder page,
        ...
        while (matcher.find(idx)) {
            ...
            String finalUrl = BrowseHelper.encodeUrl(UrlOperations.resolveUrl(baseUrl, url));
    ...
    public static String getTagAttr(StringBuilder page, final String tag,
        ...
        String found = null;
        Pattern daPattern = getPattern(tag, attr);
     ...
     *
     * @param page      StringBuilding holding HTML
     * @param tag       String containing tagname of interest
     * @param findAttr  name of attribute within the tag to return
     * @param whereAttr only match tags with an attribute whereAttr
     * @param whereVal  only match tags with whereAttr having this value
