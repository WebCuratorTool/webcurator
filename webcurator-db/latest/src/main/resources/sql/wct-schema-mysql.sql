...
create table DB_WCT.HARVEST_RESULT (HR_OID bigint not null, HR_HARVEST_NO integer, HR_TARGET_INSTANCE_ID bigint, HR_PROVENANCE_NOTE text not null, HR_CREATED_DATE datetime, HR_CREATED_BY_ID bigint, HR_STATE integer, HR_STATUS integer, HR_DERIVED_FROM integer, HR_INDEX integer, HR_RR_OID bigint, primary key (HR_OID));
...
alter table DB_WCT.VISUALIZATION_IMPORTED_FILE add index IDX_VIF_UPLOADED_DATE (VIF_UPLOADED_DATE);
