...
SELECT 	TARGET.T_AT_OID,
		ABSTRACT_TARGET.AT_OID,
...
LEFT JOIN 	(SELECT S_TARGET_ID,
	                GROUP_CONCAT(S_OID) AS SEED_IDS,
	                GROUP_CONCAT(S_SEED) AS SEED_NAMES,
	                GROUP_CONCAT(CONVERT(S_PRIMARY, UNSIGNED)) AS SEED_PRIMARIES
	        FROM   DB_WCT.SEED
...
LEFT JOIN (SELECT 	GROUP_MEMBER.GM_CHILD_ID AS TARGET_ID,
					GROUP_CONCAT(ABSTRACT_TARGET.AT_NAME) AS GROUP_NAMES
