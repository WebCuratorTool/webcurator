...
public class H3ScriptConsoleController extends AbstractCommandController {
	...
	protected ModelAndView handle(HttpServletRequest req, HttpServletResponse res, Object comm, BindException errors) throws Exception {
		...
		Profile profile = targetInstanceManager.getTargetInstance(command.getTargetInstanceOid(), true).getProfile();

		if (authorityManager.hasAtLeastOnePrivilege(profile, new String[] {Privilege.MANAGE_TARGET_INSTANCES, Privilege.MANAGE_WEB_HARVESTER})) {
