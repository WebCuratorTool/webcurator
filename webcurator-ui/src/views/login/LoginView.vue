...
const msgKey = ref(0);
const login = async () => {
  feedback.value = await authStore.authenticate(
    route.path,
    username.value,
    password.value,
  );
...
<template>
  <div
    class="surface-ground flex flex-col items-center justify-center min-h-screen min-w-screen overflow-hidden"
  >
    <div
    ...
    >
      ...
      <Button
        label="Sign In"
        class="wct-primary-button mt-4 w-full"
        @click="login"
        fluid
      />
