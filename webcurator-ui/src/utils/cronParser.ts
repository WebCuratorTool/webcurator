...
export const createCustomCronExpression = (cronSettings: {
  dayOfMonth: string;
  month: string;
  dayOfWeek: string;
  minute: string;
...
export const createCronExpression = (cronSettings: {
  dayOfMonth: string;
  months: string;
  dayOfWeek: string;
  time: string;
  ...
  if (cronSettings.months) {
    ...
    if (cronMonths.length === 1) {
      ...
      const monthNumbers = cronMonths.map(
  ...
  if (cronSettings.dayOfMonth) {
    cron.dayOfMonth =
      cronSettings.dayOfMonth === "Last" ? "L" : cronSettings.dayOfMonth;
  } else {
    if (cron.dayOfWeek === "?") {
      cron.dayOfMonth = "*";
...
export const getAlternatingMonths = () => {
  ...
  const evenMonths = months
    .filter((month, index) => index % 2 !== 0)
    .join(", ");
